...
namespace ImportStationData_FromExcel
{
   public class Import_StationMaster
    {
        ...
        public Boolean IsImportFileInValid()
        {
            ...
            try
            {
                ...
                string StationName = "";
                Int16 StationNamecol = 1;
                //ExcelPackage p = new ExcelPackage(new System.IO.FileInfo(excelfile));
                ExcelWorksheet excelsht = p.Workbook.Worksheets[sheetname];
                StationName = Convert.ToString(excelsht.Cells[r, StationNamecol].Value);
                ...
                if (StationName != "")
                {
                    ...
                    for (r = 2; r <= dimend.Row; r++)
                    {
                        ...
                        Station_Name = Convert.ToString(excelsht.Cells[r, Station_NameCol].Value);
                        Abbrr = Convert.ToString(excelsht.Cells[r, Abbrrcol].Value);
                        Pin = Convert.ToString(excelsht.Cells[r, PinCol].Value);
                        State = Convert.ToString(excelsht.Cells[r, StateCol].Value);
                        ...
                        //    isInsert = true;
                        //
                        if (lastcolumnname.ToLower() != "stn_id")
                        {
                            ...
                            if (Station_Name.Trim().Length == 0)
                            {
                                //invalidcols += "barcode, ";
                                isinvalid = true;
                                txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No:" + (r - 1).ToString() + " Required value of 'Station' is missing.";
                            }
                        ...
                        else //update
                        {
                            ...
                            {
                                if (excelsht.Cells[1, c].Value.ToString() == "Stn_Name")
                                {
                ...
                for (r = 2; r <= dimend.Row; r++)
                {
                    ...
                    if (sheetname == "Station")
                    {
                        if (ignoreerror)
                        {
                            ...
                            StationName = Convert.ToString(excelsheet.Cells[r, StationNameCol].Value);
                            Abbr = Convert.ToString(excelsheet.Cells[r, Abbrcol].Value);
                            PinCode = Convert.ToString(excelsheet.Cells[r, PinCodeCol].Value);
                            State = Convert.ToString(excelsheet.Cells[r, StateCol].Value);
                            ...
                            if (StationName.Trim().Length == 0)
                            {
                                ...
                                txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No:" + (r - 1).ToString() + " Required value of 'Station Name' is missing.";
                            }
                            ...
                            else if (PinCode.Trim().Length == 0)
                            {
                                //invalidcols += "Subgroup, ";
                                IgnorErrorExcel = true;
                                txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No:" + (r - 1).ToString() + " Required value of 'Pin' is missing.";
                            }
                            else if (State.Trim().Length == 0)
                            {
                                //invalidcols += "Subgroup, ";
                                IgnorErrorExcel = true;
                                txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No:" + (r - 1).ToString() + " Required value of 'State' is missing.";
                            }
                            if (IgnorErrorExcel)
                            {
                                trans.Rollback();
                    ...
                    if (sheetname == "Station")
                    {
                        ...
                        sql = "Select Stn_Name From Station Where Stn_Name = '" + StationName + "'";
                        DataTable dt1 = null;
                        error = GetTable(sql, trans, ref dt1);
                        if (dt1.Rows.Count > 0)
                        {
                            txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No: " + (r - 1).ToString() + " Station '" + StationName + "' Already Exists \r\n\r\n";
                            trans.Rollback();
                            goto Err;
                        }

                        sql = "Select State_Name From State Where State_Name = '" + State + "'";
                        DataTable dt2 = null;
                        error = GetTable(sql, trans, ref dt2);
                        if (dt2.Rows.Count == 0)
                        {
                            txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No: " + (r - 1).ToString() + " State '" + State + "' Does Not Exists \r\n\r\n";
                            trans.Rollback();
                            goto Err;
                        }
                    ...
                    for (c = 1; c <= dimend.Column; c++)
                    {
                        error = ReadAndCreate(excelsheet, r, c, trans, ref columnstring, ref valuestring, ref udfcolumnstring, ref udfvaluestring, ref StnId, ref StnKey);
                    }
                    ...
                    if (sheetname == "Station")
                    {
                        error = NewRecordId("Station", "Stn_Id", trans, ref StnId);
                        ...
                        columnstring = "Cobr_id,Stn_Id,Stn_Key," + columnstring + " ,City_Key, Alt_Code,Status,Created_By,Created_Dt,Updated_By,Updated_Dt,Dist_Key,RefCode,synchcloud,Auto_Code";
                        valuestring = "'" + branchid + "'," + StnId + ",'" + branchid + StnKey + "'," + valuestring + ",'','',1," + importuser + ",'" + dttime.ToString("MM/dd/yyyy hh:mm tt") + "',NULL,NULL,'','',0,''";

                        sql = "Insert into Station (" + columnstring + ") values (" + valuestring + ")";
                    ...
                    else
                    {
                        ...
                        if (sheetname == "Station")
                        {
                            ...
                            txtstatus.AppendText("\r\n Station: " + StationName + " updated successfully");
                        }
                        else
                        {
        ...
        private void UpdateEntryInTable(ExcelWorksheet excelsheet)
        {
            ...
            try
            {
                ...
                for (r = 2; r <= dimend.Row; r++)
                {
                    ...
                    StnKey = updateonvalue;

                    for (c = 1; c <= dimend.Column; c++)
                    {
                        error = UpdateReadAndCreate(excelsheet, r, c, trans, ref columnstring, ref valuestring, ref udfvaluestring, ref StnId, ref StnKey);
                        if (error)
                        {
                            goto Err;
                        }
                ...
                Err:
                    ...
                    if (!error)
                    {
                        ...
                        sql = "Update Station Set " + valuestring + " Where " + updateoncolumn + " = " + updateonvalue + "";
                        sw.WriteLine(sql);
                        error = ExecuteQuery(sql, trans);
        ...
        //
        private Boolean ReadAndCreate(ExcelWorksheet excelsheet, Int32 r, Int32 c, SqlTransaction trans, ref string columnstring, ref string valuestring, ref string udfcolumnstring, ref string udfvaluestring, ref Int32 stnid, ref Int32 stnkey)
        {
            try
            {
                string dbcolname = "";
                ...
                if (srchcols.Count != 0 || excelcolhdr == "Stn_Name" || excelcolhdr == "AltCode")
                {
                    ...
                    else if (dbcolname.ToLower() != "stn_id")
                    {
                    ...
                    if (datatype.ToLower() == "varchar" || datatype.ToLower() == "char")
                    {
                        ...
                        if (dbcolname.ToLower() == "pin")
                        {
                            Pin = val.Trim();
                        }
                        if (dbcolname.ToLower() == "state_key")
                        {
                            sql = "select State_Key From State where state_name = '" + val.Trim() + "'";
                            DataTable dt4 = null;
                            error = GetTable(sql, trans, ref dt4);
