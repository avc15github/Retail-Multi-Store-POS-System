...
namespace ImportCustomerData_FromExcel
{
   public class Import_CustomerMaster
   {
       ...
       private void CreateEntryInTable(ExcelWorksheet excelsheet)
       {
           ...
           try
           {
               ...
               for (r = 2; r <= dimend.Row; r++)
               {
                   ...
                   Customer = Convert.ToString(excelsheet.Cells[r, CustomerCol].Value);
                   ...
                   Customer = Convert.ToString(excelsheet.Cells[r, CustomerCol].Value);
                   sql = "Select Led_name From Ledger Where Ledger.Led_name = '" + Customer + "' And Led_cat='R' ";
                   DataTable dt1 = null;
                   error = GetTable(sql, trans, ref dt1);
                   if (dt1.Rows.Count > 0)
                   {
                       txterrorstatus.Text += "**********ERROR********** \r\n\r\n Line No: " + (r - 1).ToString() + "  Customer '" + Customer + "' Already Exists \r\n\r\n";
                       trans.Rollback();
                       goto Err;
                   }
...
public class ImportCustomerColumns
{
